...
void createAnimation(AnimationInfo animation, TickerProvider vsync) {
  ...
  // Flutter animate relies on each effect's duration, but a raw AnimationController
  // still needs a default duration before .forward() is called.
  final effects = animation.effects;
  if (effects.isNotEmpty) {
    final duration = effects
        .map((effect) => effect.duration ?? Duration.zero)
        .fold(Duration.zero, (prev, element) => element > prev ? element : prev);
    if (duration > Duration.zero) {
      newController.duration = duration;
    }
  }
